...
local owner = CreateFrame("Frame", "HelloDruid_BindOwner")
...
-- too. (Positions are a flat reading-order list spanning the full two-row form
-- grid.) The utility row and anything
...
local function defaults()
    return {
        ...
        -- Utility row (buffs, cooldowns, Rebirth, then racial) on Alt+1..8.
        utility = {
...
local function orderedUtility(AB)
    local t = {}
    for _, b in ipairs(AB.utilityButtons or {}) do
        if b:IsShown() then t[#t + 1] = b end
    end
    return t
end
...
function KB:Apply()
    ...
    -- off. AB:SetVisible calls Apply when toggling.
    if HelloDruidCharDB.showBars == false then return end

    local kb = HelloDruidCharDB.keybinds
    ...
    -- Bind every fixed physical ability slot, including slots hidden by the
    -- current form. The secure state driver can reveal a longer form layout in
    -- combat, when override bindings cannot be rebuilt; prebinding all slots
    -- keeps those newly visible positions live.
    bind(kb.ability, AB.buttons)
    bind(kb.utility, orderedUtility(AB))
end

function KB:SetLabel(btn, key)
    if not btn._hdKeyLabel then
        local fs = btn:CreateFontString(nil, "OVERLAY", "NumberFontNormalSmallGray")
        fs:SetPoint("TOPRIGHT", btn, "TOPRIGHT", -2, -2)
        fs:SetJustifyH("RIGHT")
        btn._hdKeyLabel = fs
    end
    btn._hdKeyLabel:SetText(key and shortKey(key) or "")
end

-- Paint every physical ability slot up front; a longer form can reveal those
-- slots in combat, when protected bindings and labels cannot be rebuilt.
function KB:RefreshLabels()
    local AB = ns.ActionBar
    if not AB or not AB.buttons then return end
    local kb = HelloDruidCharDB.keybinds

    for pos, b in ipairs(AB.buttons) do self:SetLabel(b, kb and kb.ability[pos]) end
    local so = orderedUtility(AB)
    for pos, b in ipairs(so) do self:SetLabel(b, kb and kb.utility[pos]) end
...
function KB:Assign(kind, pos, key)
    ...
    self:Apply()
    self:RefreshLabels()
    print(("|cff7acb59HelloDruid|r bound %s to %s slot %d"):format(key, kind, pos))
end
...
function KB:ClearAll()
    HelloDruidCharDB.keybinds = { ability = {}, utility = {} }
    self:Apply()
    self:RefreshLabels()
    print("|cff7acb59HelloDruid|r keybindings cleared.")
end

function KB:ResetDefaults()
    HelloDruidCharDB.keybinds = defaults()
    self:Apply()
    self:RefreshLabels()
    print("|cff7acb59HelloDruid|r keybindings reset to defaults.")
end
...
function KB:TargetUnderMouse()
    ...
    for pos, b in ipairs(orderedUtility(AB)) do
        if b == f then return "utility", pos end
    end
end
...
function KB:EnsureUI()
    ...
    banner:SetText("HelloDruid keybind mode\n" ..
...
function KB:EnterMode()
    ...
    local AB = ns.ActionBar
    if not AB or not AB.buttons then return end
    if HelloDruidCharDB.showBars == false then
        print("|cff7acb59HelloDruid|r turn the bars on first (/hd bars on) to edit keybindings.")
        return
    end
    if InCombatLockdown() then
        print("|cff7acb59HelloDruid|r can't edit keybindings in combat.")
        return
    end
    self.mode = true
    self:EnsureUI()
    ...
    self.banner:Show()
    self:RefreshLabels()
    print("|cff7acb59HelloDruid|r keybind mode: hover a button, press a key. Delete clears, Esc exits.")
end
...
-- so HelloDruidCharDB already exists). Only when absent, so cleared bindings
-- aren't silently re-added on the next login.
ns:On("ADDON_LOADED", function(name)
    if name ~= ns.ADDON_NAME then return end
    if HelloDruidCharDB.keybinds == nil then
        HelloDruidCharDB.keybinds = defaults()
    end
end)
...
ns:On("PLAYER_REGEN_DISABLED", function()
    if KB.mode then
        KB:ExitMode()
        print("|cff7acb59HelloDruid|r keybind mode closed (entered combat).")
    end
end)
