...
-- restyles that frame to look like this one for every non-Druid, and two bars
...
--
-- WHY HELLODRUID OWNS A CAST BAR AT ALL. Blizzard's PlayerCastingBarFrame is
...
--
-- Balance uses this constantly for Wrath and Starfire; it also covers channels,
-- utility casts, bandages, mounts and hearthstones in every form.
...
function CB:Build(container)
    ...
    local bar = CreateFrame("StatusBar", "HelloDruid_CastBar", container)
...
function CB:Start()
    if not self.bar or HelloDruidCharDB.showCastBar == false then return end
    ...
    colour(self.bar, 0.35, 0.75, 0.45)  -- druid green
...
function CB:Apply()
    ...
    local on = HelloDruidCharDB.showCastBar ~= false
...
-- Use Blizzard's own replacement-bar switch. This is the same supported path
-- HelloUI uses, but keeping it here means HelloDruid works correctly with the
-- currently released HelloUI, whose sibling list predates this addon.
function CB:SyncBlizzard()
    local blizzard = _G.PlayerCastingBarFrame
    if not (blizzard and blizzard.SetAndUpdateShowCastbar) then return end
    local clusterShown = not ns.ActionBar.container or ns.ActionBar.container:IsShown()
    local ours = HelloDruidCharDB.showCastBar ~= false and clusterShown
    blizzard:SetAndUpdateShowCastbar(not ours)
end

function CB:SetVisible(visible)
    HelloDruidCharDB.showCastBar = visible and true or false
    self:Apply()
